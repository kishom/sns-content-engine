...
11. **回転の支点を動かすには pivot Group でくるむ。** しっぽ/垂れ耳は「メッシュ中心」で回すと
   ...
   （表情8枚も同様に MD5 一致）。造形に触らずに可動域だけ足したいときはこの型を使う。
   - ⚠️ ただし**納品4ショット（dpr=2）だけは再レンダリングで MD5 が変わる**。これは
     **改修とは無関係の既存の非決定性**（大きいバッファ×SwiftShader・README冒頭の
     「実測 mean 0.008/255」と同じ現象）。実測 PSNR 58〜76dB＝知覚不可。
     **切り分け方**: 改修前の scene.js（`git show <改修前commit>:scripts/char3d/scene.js`）で
     同じショットを描き、改修後の出力と突き合わせる。**PSNR=inf（＝MD5一致）なら改修は無罪**。
     実際にこの方法で inuinu_front / nekoneko_front が bit 単位で一致することを確認済み。
     dpr=2 の4枚を MD5 で回帰判定してはいけない（毎回落ちる）。
